                    ...
                    <div class="list">
                        <div class="list-item" v-for="(order,orderIndex) in orderList" :class="[(orderIndex==orderList.length-1)?'list-item-last':'']" @click="toggleActiveItem(orderIndex)">
                            ...
                            <div class="dot-sign-container">
                                <image class="dot-sign" :src="order['STORE_ORDER_RETURN.OPERATOR_NAME'] == 'meituan'?meituanIcon:elmIcon"></image>
                            </div>
                            <div class="list-item-true-area" :class="[focusOrder===orderIndex?'list-item-true-area-active':'']">
                                <image class="order-return-icon" :src="returnIcon" v-if="order['STORE_ORDER_RETURN.STATE'] == 'WAIT_CANCEL'||order['STORE_ORDER_RETURN.STATE'] == 'CANCEL'"></image>
                                <div class="order-info">
                                    
                                    <div style="flex-direction:row;align-items:center">
                                        <text class="user-info">{{order['STORE_ORDER_RETURN.USER_NAME'] || '匿名'}}   #{{order['STORE_ORDER_RETURN.GET_NUM']}}</text>
                                        <image class="order-type-icon" :src="order['STORE_ORDER_RETURN.STATE'] == 'WAIT'?doingIcon:''" v-if="order['STORE_ORDER_RETURN.STATE'] == 'WAIT'"></image>
                                        <image class="order-type-icon" :src="waitconfirmIcon" v-if="order['STORE_ORDER_RETURN.STATE'] == 'WAIT_CANCEL'"></image>
                                    </div>
                                    <text class="order-time">{{order['STORE_ORDER_RETURN.ADD_DATETIME']}}</text>
                                </div>
                                <div class="item-price">
                                    <text class="item-price-text">{{order["STORE_ORDER_RETURN.PAID_MONEY"] | price}}</text>
                                </div>
                                <div class="return-sign" v-if="order['STORE_ORDER_RETURN.RETURN_MONEY']">
        ...
        <div class="billings-area">
            <div class="pay-right-now" @click='submit'>
                <div style="flex-direction:row;align-items:center;">
                    ...
                    <text class="pay-right-now-h1">{{orderTotal}}</text>
                    <text class="pay-right-now-text">  笔</text>
                </div>
                <text class="pay-right-now-h1" ref="ant">￥1231</text>
            </div>
        </div>
