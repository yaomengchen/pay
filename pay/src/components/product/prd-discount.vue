        ...
        <div class="preferential-modal" @click="stopClick">
            <div class="title">
                <div class="toggle-bar">
                    ...
                    <div class="toggle-bar-item" :class="[nowIndex==2?'toggle-bar-item-active':'']" @click="change(2)">
                        <text class="toggle-bar-item-name">单品备注</text>
                    </div>
                </div>
            ...
            <div class="toggle-bar-item-ref-area">
                ...
                <div class="swipe-container">
                    <text class="tip">{{product['PRODUCT_SKU.SKU_NAME']}}</text>   
                    <div class="type-in-area type-in-area-1" v-if="nowIndex==0">
                        <input type="number" class="type-in-item" placeholder="请填写单品折扣，例如:8折,则输入8" autofocus="true" v-model="rateText"/>
                    </div>
                    ...
                    <div v-if="nowIndex==2">
                        <div class="radio3" v-if="remarkList.length > 0">
                            <div class="radio-item3" v-for="item in remarkList" @click="setValue(item.value,'remark')">
                                <text class="radio-text">{{item.value}}</text>
                            </div>
                        </div>
                        <div class="type-in-area type-in-area-2">
                            <textarea class="textarea" placeholder="请填写单品备注" autofocus="true" v-model="remark"></textarea>
                            <text class="text-btn" @click="addRemark">添加到常用备注</text>
                        </div>
                    </div>
            ...
            <div class="computed-group">
                <div class="row">
                    ...
                    <text class="total-price">{{product['PRODUCT_SKU.ORDER_PRICE'] | price}}</text>
                </div>
    ...
    export default {
        ...
        methods: {
            ...
            submit(){
                ...
                if(self.nowIndex == 0 && self.rate != 10){
                    ...
                    storage.setItem('prd_discount_history',JSON.stringify(self.radioList))
                }
                if(this.nowIndex == 2){
                    self.$emit('event',{value:this.remark,type:'remark'})
                }else{
                    self.$emit('event',{value:this.overPrice,type:'price'})
                }                
            },
            close(){
                var self = this
            ...
            getRemark(){
                var self = this
                storage.getItem('prd_remark_history',res =>{
            ...
            addRemark(){
                if(this.remark){
                    ...
                    storage.setItem('prd_remark_history',JSON.stringify(this.remarkList),res =>{
        ...
        watch:{
            display(){
                ...
                if(this.display == false){                    
                    this.history = this.nowIndex
                    this.nowIndex = -1
                }else{
                    this.nowIndex = this.history
                    this.remark = this.product.REMARK
                }
            },
            nowIndex(){
                if(this.nowIndex == 0){
                    storage.getItem('prd_discount_history',res =>{
                        if(res.result == 'success'){
                    ...
                    this.radioList = this.getMoneyList(this.product['PRODUCT_SKU.ORDER_PRICE'])
                }else{
                    this.getRemark()
                }
            }
        ...
        computed: {
            discount(){
                var sum
                if(this.nowIndex == 0){
                    sum = this.product['PRODUCT_SKU.ORDER_PRICE'] * (10 - this.rate)/10
                }else {
                    sum = this.cutPrice
                }
                return sum
            ...
            overPrice(){
                return this.product['PRODUCT_SKU.ORDER_PRICE'] - this.discount
            },
            rate() {
